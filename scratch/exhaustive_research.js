async function exhaustiveResearch(itemId) {
    console.log('--- Exhaustive Aladin Internal Research for ID:', itemId, '---');
    const scenarios = [
    ...
    for (const scenario of scenarios) {
        ...
        if (res.ok) {
            ...
            for (const enc of encodings) {
                ...
                if (hasKeywords) {
                    console.log(`  - SUCCESS Code found with [${enc}]! Length: ${text.length}`);
                    return; // Stop on first success found
                }
            }
            console.log('  - No TOC keywords found in this response.');
        } else {
